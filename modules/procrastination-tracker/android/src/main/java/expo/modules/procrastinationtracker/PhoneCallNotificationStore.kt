...
/**
 * Best-effort fallback for devices where READ_CALL_LOG is unavailable/restricted.
 * It stores only call-like ongoing notification metadata, never arbitrary notification text.
 */
object PhoneCallNotificationStore {
  private const val PREFS = "fms_phone_notification_tracker"
    ...
    if (!found) {
      next.put(
        JSONObject()
          ...
          .put("contactLabel", contactLabel ?: "Phone call")
  ...
  fun finish(context: Context, notificationKey: String, endedAt: Long) {
    ...
    for (i in 0 until active.length()) {
      val item = active.optJSONObject(i) ?: continue
      if (item.optString("notificationKey") == notificationKey) completedItem = item else remaining.put(item)
    }
    ...
    val contactLabel = item.optString("contactLabel", "Phone call")
    val packageName = item.optString("packageName", "unknown.dialer")
    val direction = item.optString("direction", "unknown")
    val eventId = stableId("phone-notification|$packageName|$contactLabel|$startedAt|$endedAt")
    ...
    for (i in 0 until completed.length()) {
      ...
      results.add(
        mapOf(
          ...
          "contactLabel" to item.optString("contactLabel", "Phone call"),
